...
class LihatSemuaKandidatPageDetailArgument {
  int index;
  LihatSemuaKandidatPageDetailArgument({required this.index});
}

class LihatSemuaKandidatPageDetail extends StatefulWidget {
  static const String id = 'LihatSemuaKandidatPageDetail';
  LihatSemuaKandidatPageDetailArgument args;
  LihatSemuaKandidatPageDetail({Key? key, required this.args})
      : super(key: key);

  @override
  State<LihatSemuaKandidatPageDetail> createState() =>
      _LihatSemuaKandidatPageDetailState();
}

class _LihatSemuaKandidatPageDetailState
    extends State<LihatSemuaKandidatPageDetail> {
  ...
  void confirmationDiscualification(NeighbourhoodHeadCandidate data) async {
    final _alasanController = TextEditingController();
    showDialog<String>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        ...
        content: Text(
          'Apakah anda yakin mendiskualifikasi kandidat ${data.dataUser!.full_name}? \n\n*Anda wajib mengisikan alasan kandidat terdiskualifikasi!',
          style: smartRTTextNormal.copyWith(fontWeight: FontWeight.normal),
        ),
        actions: <Widget>[
          Padding(
  ...
  void getData({required NeighbourhoodHeadCandidate dataKandidat}) async {
    ...
    status = dataKandidat.status == -2
        ...
        : dataKandidat.status == -1
            ? 'Diskualifikasi\n${DateFormat('d MMMM y', 'id_ID').format(dataKandidat.discualified_at!)}'
            : 'Aktif';

    if (dataKandidat.status == -1) {
      diDiskualifikasiOleh = dataKandidat.discualified_by!.full_name;
    }
  ...
  Widget build(BuildContext context) {
    ...
    NeighbourhoodHeadCandidate dataKandidat = listKandidat[index];
    ...
    return Scaffold(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (user.is_committe == 1 || user.id == dataKandidat.dataUser!.id)
                ListTileData1(
              ...
              if (diDiskualifikasiOleh != '' && user.is_committe == 1)
                ListTileData1(
                    txtLeft: 'Diskualifikasi Oleh',
      ...
      bottomNavigationBar:
          (dataKandidat.status == 1 && user.id == dataKandidat.dataUser!.id)
              ...
              : (dataKandidat.status == 1 && user.is_committe == 1)
                  ? Container(
                      padding: paddingScreen,
                      width: double.infinity,
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          confirmationDiscualification(dataKandidat);
                        },
                        child: Text(
