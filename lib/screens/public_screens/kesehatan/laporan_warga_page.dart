...
class _LaporanWargaPageState extends State<LaporanWargaPage> {
  List<UserHealthReport> listMenungguKonfirmasi = [];
  List<UserHealthReport> listTelahDikonfirmasi = [];

  void getData() async {
    Response<dynamic> resp;
    resp = await NetUtil()
        .dioClient
        .get('/health/userReported/all/status/belumDikonfirmasi');
    listMenungguKonfirmasi.addAll((resp.data).map<UserHealthReport>((request) {
      return UserHealthReport.fromData(request);
    }));

    resp = await NetUtil()
        .dioClient
        .get('/health/userReported/all/status/telahDikonfirmasi');
    listTelahDikonfirmasi.addAll((resp.data).map<UserHealthReport>((request) {
      return UserHealthReport.fromData(request);
    }));
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: <Widget>[
            listMenungguKonfirmasi.isNotEmpty
                ? ListView.separated(
                        ...
                        onTap: () {
                          DetailRiwayatKesehatanArguments arguments =
                              DetailRiwayatKesehatanArguments(
                                  dataReport: listMenungguKonfirmasi[index]);
                          Navigator.pushNamed(
                    ...
                    itemBuilder: (context, index) {
                      return CardListTileWithStatusColor(
                        ...
                        onTap: () {
                          DetailRiwayatKesehatanArguments arguments =
                              DetailRiwayatKesehatanArguments(
                                  dataReport: listTelahDikonfirmasi[index]);
                          Navigator.pushNamed(
                              context, DetailRiwayatKesehatanPage.id,
                              arguments: arguments);
                        },
